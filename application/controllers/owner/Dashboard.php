...
class Dashboard extends CI_Controller {
    ...
    //  $this->user_login_authentication();
      ...
      $this->load->model('owner_models/dashboard_model', 'dash_mdl');	
      $this->load->model('owner_models/booktrip_model', 'booktrip_mdl');	
      $this->load->model('owner_models/customer_model', 'customer_mdl'); 
      $this->load->model('owner_models/trip_model', 'trip_mdl'); 
    ...
    public function index() {
      ...
      $data['trip_info'] = $this->trip_mdl->get_trip_info($data['loginId']);
      ...
      $data['upcoming'] = $this->booktrip_mdl->count_total_upcoming_trip($data['loginId']); 
      $data['ongoing'] = $this->booktrip_mdl->count_total_ongoing_trip($data['loginId']); 
      $data['completed'] = $this->booktrip_mdl->count_total_completed_trip($data['loginId']); 
      $data['cancel'] = $this->booktrip_mdl->count_total_cancel_trip($data['loginId']);
      $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
      $data['main_content'] = $this->load->view('owner_views/dashboard/dashboard_v', '', TRUE);
    ...
    public function edit_trip($trip_id) { 
       ...
       // echo '<pre>' ;print_r($data['tripData']);die;
        if (!empty($data['tripData'])) { 
            ...
            $data['active_sub_sub_menu'] = ''; 
            $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
            $data['main_content'] = $this->load->view('owner_views/dashboard/edit_trip_v', $data, TRUE);
            $this->load->view('owner_views/admin_master_v', $data); 
        } else { 
            $sdata['exception'] = 'Trip not found !'; 
            $this->session->set_userdata($sdata); 
    ...
    public function view_trip($trip_id) { 
       ...
       // echo '<pre>' ;print_r($data['user_data']);
        if (!empty($data['user_data'])) { 
            ...
            $data['active_sub_sub_menu'] = ''; 
            $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
            $data['main_content'] = $this->load->view('owner_views/dashboard/view_trip_v', $data, TRUE);
            $this->load->view('owner_views/admin_master_v', $data); 
        } else { 
            $sdata['exception'] = 'Trip not found !'; 
            $this->session->set_userdata($sdata); 
