...
//        $this->user_login_authentication(); 
    ...
    public function index() {
        ...
        $data['trip_info'] = $this->trip_mdl->getAllTripData($data['loginId']);
       // echo '<pre>' ;        print_r($data['trip_info']);die;
        $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
        $data['main_content'] = $this->load->view('owner_views/trips/manage_trip_v', $data, TRUE);
    ...
    public function edit_trip($trip_id) { 
       ...
       // echo '<pre>' ;print_r($data['tripData']);die;
        if (!empty($data['tripData'])) { 
            ...
            $data['active_sub_sub_menu'] = ''; 
            $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
            $data['main_content'] = $this->load->view('owner_views/trips/edit_trip_v', $data, TRUE);
            $this->load->view('owner_views/admin_master_v', $data); 
        } else { 
            $sdata['exception'] = 'Trip not found !'; 
            $this->session->set_userdata($sdata); 
    ...
    public function view_trip($trip_id) { 
        ...
        if (!empty($data['user_data'])) { 
            ...
            $data['active_sub_sub_menu'] = ''; 
            $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
            $data['main_content'] = $this->load->view('owner_views/trips/view_trip_v', $data, TRUE);
            $this->load->view('owner_views/admin_master_v', $data); 
        } else { 
            $sdata['exception'] = 'Trip not found !'; 
            $this->session->set_userdata($sdata); 
     ...
     public function trip_by_status($tstatus) {
        ...
        $data['active_menu'] = 'trip';
        $data['loginId']=$this->session->userdata('admin_id'); 
        $data['trip_data'] = $this->trip_mdl->get_trip_data_by_status($tstatus,$data['loginId']);
        $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
        $data['main_content'] = $this->load->view('owner_views/trips/trip_by_status_v', $data, TRUE);
        $this->load->view('owner_views/admin_master_v', $data);
    }
...
//        }
        ...
        $data['loginId']=$this->session->userdata('admin_id'); 
        $data['trip_info'] = $this->trip_mdl->getAllTripRequestHistory($data['loginId']);
       // echo '<pre>' ;        print_r($data['trip_info']);die;
        $data['main_menu'] = $this->load->view('owner_views/main_menu_v', $data, TRUE);
        $data['main_content'] = $this->load->view('owner_views/trips/request_history', $data, TRUE);
        $this->load->view('owner_views/admin_master_v', $data);
    } 
